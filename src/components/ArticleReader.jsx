...
const ArticleReader = ({ article, onBack, allArticles, setHeaderContent }) => {
  ...
  // 6. Mouse Hover Focus (Fallback for when Eye Tracking is off)
  const handleMouseMove = (e) => {
      if (isEyeTracking || isCalibrating) return; // Leave it to gaze 

      const y = e.clientY;
      let foundIndex = -1;
      
      // Throttle DOM queries slightly for performance
      requestAnimationFrame(() => {
          ...
          if (foundIndex !== focusedParagraphIndex) {
              setFocusedParagraphIndex(foundIndex);
          }
      });
  };
